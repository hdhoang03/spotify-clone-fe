...
import { useState, useEffect } from 'react';
import { X, Search, Plus, Check, Music, Loader2 } from 'lucide-react';
import api from '../../../services/api';
import type { SongResponse } from '../../../types/backend';
...
const AddSongToAlbumModal = ({ isOpen, onClose, currentSongIds, onAdd }: AddSongToAlbumModalProps) => {
    const [searchTerm, setSearchTerm] = useState('');
    const [songs, setSongs] = useState<SongResponse[]>([]);
    const [isLoading, setIsLoading] = useState(false);
    const [isAddingId, setIsAddingId] = useState<string | null>(null);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
            ...
            <div className="relative z-10 bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl w-full max-w-lg border border-gray-100 dark:border-white/10 flex flex-col max-h-[85vh]">
                <div className="flex items-center justify-between p-5 md:p-6 border-b border-gray-100 dark:border-white/5">
                    <div>
                        <h2 className="text-xl font-bold text-gray-900 dark:text-white">Thêm vào Album</h2>
                    </div>
                    <button onClick={onClose} className="p-2 text-gray-400 hover:text-gray-600 rounded-full transition"><X size={20} /></button>
                </div>
                ...
                <div className="p-5 pb-2">
                    <div className="relative">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
                        <input
                            type="text" placeholder="Tìm tên bài hát..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
                            className="w-full pl-10 pr-4 py-3 rounded-xl border border-gray-200 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 outline-none text-sm"
                        />
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-5 custom-scrollbar">
                    {isLoading ? (
                        <div className="flex justify-center items-center py-10"><Loader2 className="animate-spin text-orange-500" size={30} /></div>
                    ) : songs.length > 0 ? (
                        <div className="space-y-2">
                            {songs.map(song => {
                                const isAdded = currentSongIds.includes(song.id);
                                return (
                                    <div key={song.id} className="flex items-center justify-between p-3 rounded-xl border border-gray-100 dark:border-zinc-800 hover:bg-gray-50 dark:hover:bg-zinc-800/50">
                                        ...
                                        <button
                                            onClick={() => handleAdd(song.id)}
                                            disabled={isAdded || isAddingId === song.id}
                                            className={`ml-4 px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-1.5 transition ${isAdded
                                                ? 'bg-gray-100 text-gray-400 dark:bg-zinc-800 dark:text-zinc-500 cursor-not-allowed'
                                                : 'bg-orange-50 text-orange-600 hover:bg-orange-500 hover:text-white dark:bg-orange-900/20 dark:text-orange-400 dark:hover:bg-orange-600 dark:hover:text-white'
                                                }`}
                                        >
                                            {isAdded ? <><Check size={16} /> Đã có</> :
                                                isAddingId === song.id ? <Loader2 size={16} className="animate-spin" /> : <><Plus size={16} /> Thêm</>}
                                        </button>
                                    </div>
                                );
                            })}
                        ...
                        <div className="text-center py-10 text-gray-500"><Music size={40} className="mx-auto mb-2 opacity-20" /><p>Không tìm thấy</p></div>
                    )}
                </div>
            </div>
        </div>
