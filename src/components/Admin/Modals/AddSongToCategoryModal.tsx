...
import { X, Search, Plus, Music, Loader2 } from 'lucide-react';
...
const AddSongToCategoryModal = ({ isOpen, onClose, currentSongIds, onAdd }: AddSongModalProps) => {
    ...
    useEffect(() => {
        ...
        const fetchSongs = async () => {
            setIsLoading(true);
            try {
                const res = await api.get('/song/advanced-search', {
                    params: { keyword: searchTerm, page: 1, size: 50 }
                });
    ...
    const availableSongs = songs.filter(song => !currentSongIds.includes(song.id));
    ...
    return (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
            {/* Lớp nền mờ - Áp dụng đúng CSS bạn yêu cầu và sự kiện click đóng */}
            <div
                className="absolute inset-0 bg-black/60 backdrop-blur-sm transition-opacity -mt-20"
                onClick={onClose}
                aria-hidden="true"
            />
            ...
            <div className="relative z-10 bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl w-full max-w-lg border border-gray-100 dark:border-white/10 flex flex-col max-h-[85vh]">
                <div className="flex items-center justify-between p-5 md:p-6 border-b border-gray-100 dark:border-white/5">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Tìm kiếm và chọn bài hát từ thư viện</p>
                    </div>
                    <button onClick={onClose} className="p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-full transition"><X size={20} /></button>
                </div>
                ...
                <div className="p-5 md:p-6 pb-2">
                    <div className="relative">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
                        <input
                            type="text" placeholder="Tìm theo tên bài hát hoặc nghệ sĩ..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
                            className="w-full pl-10 pr-4 py-3 rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-zinc-800/50 text-gray-900 dark:text-white focus:ring-2 focus:ring-pink-500 outline-none transition text-sm"
                        />
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-5 md:p-6 pt-2 custom-scrollbar">
                    {isLoading ? (
                        <div className="flex justify-center items-center py-10"><Loader2 className="animate-spin text-pink-500" size={30} /></div>
                    ) : availableSongs.length > 0 ? (
                        <div className="space-y-2">
                            {availableSongs.map(song => (
                                <div key={song.id} className="flex items-center justify-between p-3 rounded-xl border border-gray-100 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-zinc-800/50 transition">
                                    <div className="flex items-center gap-3 overflow-hidden">
                                        <img src={song.coverUrl} alt="" className="w-12 h-12 rounded-lg object-cover flex-shrink-0" />
                                        <div className="min-w-0">
                                            <p className="text-sm font-bold text-gray-900 dark:text-white truncate">{song.title}</p>
                                            <p className="text-xs text-gray-500 truncate">{song.artist} • {song.category || 'Chưa phân loại'}</p>
                                        </div>
                                    </div>
                                    <button
                                        onClick={() => handleAddClick(song.id)}
                                        disabled={isAddingId === song.id}
                                        className="p-2 ml-4 bg-pink-50 text-pink-600 hover:bg-pink-600 hover:text-white rounded-full transition-colors dark:bg-pink-900/20 dark:text-pink-400 dark:hover:bg-pink-600 dark:hover:text-white disabled:opacity-50"
                                    >
                                        {isAddingId === song.id ? <Loader2 size={18} className="animate-spin" /> : <Plus size={18} />}
                                    </button>
                                </div>
                            ))}
                        </div>
