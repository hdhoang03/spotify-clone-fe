...
const PlaylistDetailPage = () => {
    ...
    if (isDeleting || isLoading) return (
        <Suspense fallback={<div className="h-screen bg-white dark:bg-[#121212]" />}>
            <PlaylistSkeleton trackCount={8} />
        </Suspense>
    );
    if (!playlist) return <div className="h-screen bg-white dark:bg-[#121212] flex items-center justify-center text-zinc-900 dark:text-white">{t('playlist.not_found')}</div>;

    // const isOwner = true; // TODO: Logic so sánh userId hiện tại với playlist.user.id

    const handleDeletePlaylist = async () => {
        if (window.confirm(t('playlist.delete_confirm'))) {
            try {
                if (!id) return;
                setIsDeleting(true);
    ...
    const handlePlaySongAtIndex = (index: number) => {
        if (songs && songs.length > 0) {
            playPlaylist(songs, index, playlist.name);
        }
    };

    const handlePlayPlaylist = () => {
        if (songs && songs.length > 0) {
            playPlaylist(songs, 0, playlist.name);
        }
    };
    ...
    const { currentSong, isPlaying: globalIsPlaying, togglePlay } = useMusic();

    const isThisPlaylistActive = songs?.some((song: any) => song.id === currentSong?.id);

    return (
        <div className="min-h-screen bg-white dark:bg-[#121212] relative overflow-hidden transition-colors duration-500">

            {/* HeroBackground: màu chủ đạo từ ảnh bìa + gradient fade */}
            <div className="absolute top-0 left-0 w-full" style={{ height: 600 }}>
                <HeroBackground imageUrl={playlist?.coverUrl} bleedPx={0} />
            </div>
            ...
            <div className="pt-2 relative z-10">
                ...
                <PlaylistActionBar
                    ...
                    isPlaying={isThisPlaylistActive && globalIsPlaying}
                ...
                {hasMore && (
                    <div className="flex justify-center pb-32 pt-10">
                        ...
                        >
                            ...
                            {isLoadingSongs ? t('playlist.loading') : t('playlist.load_more')}
                        </button>
                    </div>
                )}
