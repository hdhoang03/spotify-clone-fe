...
export const AICompanion: React.FC = () => {
    ...
    const [messages, setMessages] = useState<Message[]>(() => {
        ...
        return [
            {
                role: 'model',
                text: 'Chào bạn! Mình là Springtunes AI đây. Mình có thể dịch lời, kể chuyện về nghệ sĩ, hoặc tìm cho bạn một giai điệu hợp mood hôm nay. Bạn muốn nghe gì nào?'
            }
        ];
    });
    ...
    const handleSend = async (textToSend?: string) => {
        ...
        try {
            ...
            let systemInstruction = 
                `Bạn là Springtunes AI, một người bạn đồng hành nghe nhạc thân thiện, hiểu biết, trò chuyện tự nhiên như một người bạn (xưng hô là "mình" và "bạn", hoặc "AI" tuỳ lúc). ` +
            ...
            if (systemSongs.length > 0) {
                const songsContext = systemSongs.map(s => `- ID: ${s.id} | "${s.title}" (bởi ${s.artist})`).join('\n');
    ...
    const triggerQuickAction = (actionType: 'explain' | 'translate' | 'recommend' | 'chat') => {
        if (!currentSong && actionType !== 'chat') return;
