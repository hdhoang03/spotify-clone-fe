...
import { Globe, Shield, UserX, PlayCircle } from 'lucide-react';
import Footer from '../HomePage/Footer';
import SettingsSection from '../Settings/SettingsSection';
import SettingsToggle from '../Settings/SettingsToggle';
import BlockedListModal from '../Settings/BlockedListModal';
...
const SettingsPage = () => {
    ...
    // Gọi Custom Hook Privacy
    const { isPublicProfile, setIsPublicProfile, isUpdatingPrivacy, togglePrivacy } = usePrivacySettings();
    ...
    return (
        <div>
            <div className="p-4 md:p-10 pb-32 max-w-4xl mx-auto text-zinc-900 dark:text-white">
                ...
                <SettingsSection title="Phát nhạc" icon={<PlayCircle size={20} />}>
                    <SettingsToggle
                        label="Tự động phát (Autoplay)"
                        desc="Tự động phát các bài hát tương tự khi danh sách phát hiện tại kết thúc."
                        checked={settings.autoplay}
                        onChange={() => updateSetting('autoplay', !settings.autoplay)}
                    />
                </SettingsSection>

                <SettingsSection title="Quyền riêng tư" icon={<Shield size={20} />}>
                    <SettingsToggle
                        label="Hồ sơ công khai"
                        desc="Cho phép người khác tìm thấy, xem followers và danh sách phát của bạn."
                        checked={isPublicProfile}
                        onChange={togglePrivacy} // GỌI HÀM TỪ HOOK
                    ...
                    >
                        <div className="pr-4 flex-1">
                            <p className="font-medium text-zinc-900 dark:text-white group-hover:text-green-500 transition-colors">
                                Quản lý danh sách chặn
                            </p>
                            <p className="text-xs text-zinc-500 mt-0.5">
                                Xem và bỏ chặn những người dùng bạn đã giới hạn
                            </p>
                        </div>
                        <div className="p-2 bg-zinc-100 dark:bg-zinc-800 rounded-full text-zinc-600 dark:text-zinc-300 group-hover:bg-green-500 group-hover:text-white transition-colors">
                            <UserX size={18} />
                        </div>
                    </div>
                </SettingsSection>

                <SettingsSection title="Hiển thị" icon={<Globe size={20} />}>
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between py-3 gap-2">
                        <div className="flex-1">
                            <p className="font-medium">Ngôn ngữ ứng dụng</p>
                            <p className="text-xs text-zinc-500 mt-0.5">Thay đổi ngôn ngữ (Đang phát triển)</p>
                        </div>
